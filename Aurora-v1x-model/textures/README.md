...
レギンスを身体テクスチャへ移しました。身体と同一メッシュ上に描画されるため、腰ラップとの
メッシュ干渉が発生しません。

- `body-skin-default-texture.png`：VRoid Studioから書き出した身体の標準テクスチャ
- `body-skin-uv-guide.png`：身体テクスチャの2048×2048 UVガイド
- `body-leggings-v1.png`：左右の脚UVだけをチャコールブラックにした透過レイヤー

作業モデルの「全身」カスタムアイテムへ`body-leggings-v1.png`を追加し、ボトムス内の
「ズボン（折り目なし）」テクスチャレイヤーは非表示にしています。腰ラップと靴は従来の
衣装アイテムを使用します。
