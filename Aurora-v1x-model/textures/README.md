...
- `inner-black-top-v13.png`：胸部メッシュの干渉を避けるため、襟と腰側だけを残した採用テクスチャ
- `body-inner-black-v1.png`：透明化した胸部を体テクスチャ側で補う初期版
- `body-inner-black-v4.png`：首から肩の内側へ自然に広がり、腕UVへ干渉しない採用版
- `inner-black-top-v1.prompt.md`：生成プロンプト、後処理、読み込み手順

現在の統合トップスでは`inner-black-top-v13.png`を使用します。Tシャツメッシュは襟と
腰側だけを残し、ジャケットと干渉する胸部を透明化しました。黒インナーの中央部分は
`body-inner-black-v4.png`を体テクスチャへ重ねることで補完しているため、見た目を保った
まま、胸元で内側の衣装メッシュが突き抜ける問題を回避しています。
