...
パーカー型メッシュをジャケットとして使いながらフードだけを消すため、元テクスチャの
アルファを連結領域として解析しました。次の2つの独立したUV島だけを透明化し、
`outer-shirt-jacket-hoodless-v2.png`を作成しています。

- 外側フード：x=485〜1564、y=14〜626
- 内側フード：x=680〜1368、y=638〜854

矩形で周辺を消すのではなく、元アルファ上の各連結成分だけを処理しているため、隣接する
袖UVには影響しません。VRoid Studioでは旧フード付きレイヤーを削除し、フードレス版と
非表示のデフォルト画像だけに整理して保存済みです。
